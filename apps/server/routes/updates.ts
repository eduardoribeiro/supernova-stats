...
async function updateStats(packageName: string, projectName: string, data: GenericObject<number>) {
  ...
  return new Promise<void>((resolve, reject) => {
    Object.keys(data).forEach(component => {
      db.run(
        'INSERT INTO components (packageId, projectId, name, amount, date) VALUES (?, ?, ?, ?, ?)',
        packageId,
        projectId,
        component,
        data[component],
  ...
  return new Promise<void>((resolve, reject) => {
    componentsUsage.forEach(component => {
      data[component].instances.forEach(async details => {
        ...
        db.run(
          'INSERT INTO usage (componentId, projectId, file, propsSpread, moduleName, importedType, props) VALUES (?, ?, ?, ? , ?, ?, ?)',
          [componentId, projectId, file, propsSpread, moduleName, importedType, props],
          (err: Error | null) => {
            if (err) reject(err)
            else resolve()
          },
